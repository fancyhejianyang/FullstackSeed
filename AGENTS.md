...
7. **强制自动提交（必做项）**：每次 AI 完成代码或文档改动并通过约定验证后，**必须**自动执行 git commit 并填写描述本次改动的 commit message（不允许空 message，message 需概括改动内容，如 `docs: 强化自动提交规则`）；提交前仅暂存本次 AI 实际修改的文件，若工作区存在用户已有未提交改动，必须保持不纳入提交。仅当用户明确要求“不提交”时才可跳过。
